...
export const SETTINGS_REGISTRY: readonly SettingDefinition[] = [
  ...
  { id: "workspaceLayout", category: "workspace", label: "Workspace layout", description: "The arrangement of preview and editing panes.", scope: "application", control: { kind: "select", options: (["conversion", "palette", "dithering", "tilemap", "editor", "inspection", "custom"] as WorkspaceLayoutId[]).map((value) => ({ value, label: value[0]!.toUpperCase() + value.slice(1) })) }, defaultValue: "conversion", validate: oneOf(["conversion", "palette", "dithering", "tilemap", "editor", "inspection", "custom"] as const, "conversion") },
