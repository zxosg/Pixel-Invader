	...
	for(x=0; x<w; x+=chw) {
		...
		for(j=0; j<chh; j+=2) {
			for(i=0; i<chw; i+=2) {
				...
				px[k]=floor(p/ink);
				pa[l  ]=(getPixel(x+i  ,y+j  )==ink)*255;
				pa[l+1]=(getPixel(x+i+1,y+j  )==ink)*255;
				pa[l+2]=(getPixel(x+i  ,y+j+1)==ink)*255;
				pa[l+3]=(getPixel(x+i+1,y+j+1)==ink)*255;
				k++;
				l+=4;
			}
		}
...
function makeZX() {
	...
	run("Canvas Size...", "width=" + 768*2 + chLen + " height=1 position=Top-Right zero");
	outID = getImageID();
}
...
function matchPixl() {
	...
	for(n=0; n<chNum; n++) {
		...
		for(j=0; j<chh; j+=2) {
			for(i=0; i<chw; i+=2) {
				r = r + (pa[l  ]-getPixel(x+i  ,o+y+j  ));
				r = r + (pa[l+1]-getPixel(x+i+1,o+y+j  ));
				r = r + (pa[l+2]-getPixel(x+i  ,o+y+j+1));
				r = r + (pa[l+3]-getPixel(x+i+1,o+y+j+1));
				l+=4;
			}
		}
		if(r==0) {
