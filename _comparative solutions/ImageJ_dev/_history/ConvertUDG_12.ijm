		...
		for(y=0; y<6144; y+=8) {
			...
			for(s=0; s<8; s+=2) {
				for(x=0; x<8; x+=2) {
					t=y+s;
					o=0;
					w[0] = getPixel(x+o,t)+getPixel(x+o+1,t)+getPixel(x+o,t+1)+getPixel(x+o+1,t+1);
					o=8;
					w[1] = getPixel(x+o,t)+getPixel(x+o+1,t)+getPixel(x+o,t+1)+getPixel(x+o+1,t+1);
					o=16;
					w[2] = getPixel(x+o,t)+getPixel(x+o+1,t)+getPixel(x+o,t+1)+getPixel(x+o+1,t+1);
					d = d + abs(w[2]-w[0]) + abs(w[1] - w[0]);
				}
			}
			tilDbls[ctr] = tilDbls[ctr] + d;
			ctr++;
		}
