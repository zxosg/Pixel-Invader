...
// the last BufferedImage we computed Lab buffers for:
private BufferedImage labSource = null;

/** 
 * Ensures Lbuf/abuf/bbuf are allocated & filled with CIELAB values for every pixel of `image`. 
 */
private void ensureLabBuffers(BufferedImage image) {
    int w = image.getWidth(), h = image.getHeight();
    // (re)allocate if size changed or never allocated
    if (imgW != w || imgH != h || Lbuf == null || abuf == null || bbuf == null) {
        imgW = w; 
        imgH = h;
        int n = w * h;
        Lbuf  = new float[n];
...
/**
 * Precomputes a small paletteLab[][] array so that paletteLab[i] = Lab of palette[i].
 */
private void computePaletteLab(Color[] palette) {
    paletteLab = new float[palette.length][3];
    for (int i = 0; i < palette.length; i++) {
        Color c = palette[i];
        float[] lab = rgbToLab(c.getRed(), c.getGreen(), c.getBlue());
        paletteLab[i][0] = lab[0];
...
/**
 * Converts sRGB (0–255) to CIELAB (D65, Observer 2°).
 * @return float[]{ L*, a*, b* }
 */
private float[] rgbToLab(int r, int g, int b) {
    // normalize to [0,1]
    ...
    // reference white (D65)
    final float Xn = 0.95047f, Yn = 1.00000f, Zn = 1.08883f;
    // f(t) helper
    float fx = X/Xn > 0.008856f
             ? (float)Math.cbrt(X/Xn)
             : (7.787f*(X/Xn)) + (16f/116f);
    float fy = Y/Yn > 0.008856f
             ? (float)Math.cbrt(Y/Yn)
             : (7.787f*(Y/Yn)) + (16f/116f);
    float fz = Z/Zn > 0.008856f
             ? (float)Math.cbrt(Z/Zn)
             : (7.787f*(Z/Zn)) + (16f/116f);
    float L = (116f * fy) - 16f;
    float a2 = 500f * (fx - fy);
    float b2 = 200f * (fy - fz);
    return new float[]{L, a2, b2};
}
