...
public class OsgDither_dev implements PlugInFilter {
...
//    		IJ.run("Tile");
    ...
    float[] errorBufferR = new float[width + 6];
    float[] errorBufferG = new float[width + 6];
    float[] errorBufferB = new float[width + 6];
    ...
    float d100 = (float) dither / 100, d1600 = d100 / 16;
    ...
    for (int y = 0; y < height; y++) {
        ...
        for (int xx = 0; xx < width; xx++) {
            int x = leftToRight ? xx : width - 1 - xx;
            int i = offset + x;

            // Extract RGB values
            int c = pixels[i];
            int r = gtab[(c >> 16) & 0xff];
            int g = gtab[(c >> 8) & 0xff];
            int b = gtab[c & 0xff];

            // Add error correction
            r = clamp(r + Math.round(errorBufferR[x]), 0, 255);
            g = clamp(g + Math.round(errorBufferG[x]), 0, 255);
            b = clamp(b + Math.round(errorBufferB[x]), 0, 255);
            ...
            for (int idx = 0; idx < mapSize; idx++) {
                int rp = Rpal[idx] & 0xff;
                int gp = Gpal[idx] & 0xff;
                int bp = Bpal[idx] & 0xff;

                int dist = (r - rp) * (r - rp) + (g - gp) * (g - gp) + (b - bp) * (b - bp);
                if (dist < minDistance) {
                    minDistance = dist;
                    bestIndex = idx;
                }
            ...
            dthpix[i] = (byte) bestIndex;

            // Propagate error
            int rp = Rpal[bestIndex] & 0xff;
            int gp = Gpal[bestIndex] & 0xff;
            int bp = Bpal[bestIndex] & 0xff;

            float rErr = (r - rp) * d1600;
            float gErr = (g - gp) * d1600;
            float bErr = (b - bp) * d1600;

            errorBufferR[x + 1] += rErr * 7;
            errorBufferG[x + 1] += gErr * 7;
            errorBufferB[x + 1] += bErr * 7;

            if (x > 0) {
                errorBufferR[x - 1] += rErr * 3;
                errorBufferG[x - 1] += gErr * 3;
                errorBufferB[x - 1] += bErr * 3;
            }
        }
...
private int clamp(int value, int min, int max) {
    ...
    public int __ditherRGBImage(ImageProcessor ip) {
		int[] pixels = (int[])ip.getPixels();
		int width 	= ip.getWidth();
		int height 	= ip.getHeight();
...
public int patternRGBImage(ImageProcessor ip, int pattern) {
    ...
    // Initialize pattern and levels
    iniPattern(pattern);
    makeLevelTable(pattLevels);

    // Cache pattern in a 1D array
    int[] patternCache = new int[pattX * pattY];
    for (int y = 0; y < pattY; y++) {
        for (int x = 0; x < pattX; x++) {
            patternCache[y * pattX + x] = p[y][x];
        }
    }
    ...
    IntStream.range(0, height).parallel().forEach(y -> {
        ...
        for (int x = 0; x < width; x++) {
            ...
            int bestCol1 = 0, bestCol2 = 0, bestPattern = 0;
            int minDst = Integer.MAX_VALUE;

            for (int c1 = 0; c1 < mapSize; c1++) {
                int r1 = Rpal[c1] & 0xff, g1 = Gpal[c1] & 0xff, b1 = Bpal[c1] & 0xff;

                for (int c2 = c1; c2 < mapSize; c2++) {
                    int r2 = Rpal[c2] & 0xff, g2 = Gpal[c2] & 0xff, b2 = Bpal[c2] & 0xff;

                    for (int level = 0; level < pattLevels; level++) {
                        int weightedR = ptab[pattLevels - level][r1] + ptab[level][r2];
                        int weightedG = ptab[pattLevels - level][g1] + ptab[level][g2];
                        int weightedB = ptab[pattLevels - level][b1] + ptab[level][b2];

                        int rDiff = r - weightedR;
                        int gDiff = g - weightedG;
                        int bDiff = b - weightedB;
            ...
            // Apply pattern
            int patternIdx = (y % pattY) * pattX + (x % pattX);
            dthpix[i] = (patternCache[patternIdx] > bestPattern) ? (byte) bestCol1 : (byte) bestCol2;
        }
    });
    ...
    public int __patternRGBImage(ImageProcessor ip, int pattern) {
		int[] pixels = (int[])ip.getPixels();
		int width 	= ip.getWidth();
		int height 	= ip.getHeight();
